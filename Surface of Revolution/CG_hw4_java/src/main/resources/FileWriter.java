...
public class FileWriter {
    ...
    public static void writeCoordinates(BufferedWriter writer, ArrayList<List<List<Double>>> newCoordinates) throws IOException{
        ...
        for (List<List<Double>> newCoordinate: newCoordinates){
            for (List<Double> coords: newCoordinate){
                String cordX = String.format("%.8f", coords.get(0));
                String cordY = String.format("%.8f",coords.get(1));
                String cordZ = String.format("%.8f", coords.get(2));
    ...
    public static void writeFile(String out, ArrayList<List<List<Double>>> newCoordinates,
                                 List<List<Integer>> triangleCoordinates, ArrayList<List<List<Double>>> normalCoordinates) throws Exception{
    ...
    public static void writeFile(String out, ArrayList<List<List<Double>>> newCoordinates,
                                 List<List<Integer>> triangleCoordinates) throws Exception{
        try(BufferedWriter writer = Files.newBufferedWriter(Paths.get(out))){
            writer.write(first);
            writer.newLine();
            writer.newLine();
